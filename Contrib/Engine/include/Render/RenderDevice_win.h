...
class ShaderProgram;
class ShaderProgramDX8;

class RenderDeviceImpl
	: public RenderDeviceInterface
{
	LPDIRECT3D8 _d3d;
	LPDIRECT3DDEVICE8 _d3dDevice;
	...
	IDirect3DVertexBuffer8* _tempBuffer;
	...
	DWORD _currentColor;
	LPDIRECT3DBASETEXTURE8 _currentTexture;
	...
	typedef std::set<ShaderProgramDX8*> Shaders;
	Shaders _shaders;
		// список шейдеров

	LPDIRECT3DSURFACE8 _depthsurf;
	...
	BlendMode _currentBlendMode;
		// текущий режим блендинга; сохраняется для выяснения текущего режима в состоянии premultiplied alpha
